...
void initialize_skills()
{
  ...
  SPEC_SKILL_ADD(CLASS_DREADLORD, 31, 95, SPEC_SHADOWLORD);
  ...
  SPEC_SKILL_ADD(CLASS_ASSASSIN, 35, 75, SPEC_ASSMASTER);
  SPEC_SKILL_ADD(CLASS_ROGUE, 35, 75, SPEC_ASSASSIN);
  ...
  SPEC_SKILL_ADD(CLASS_RANGER, 56, 70, SPEC_BLADEMASTER);
  SKILL_ADD(CLASS_REAVER, 56, 70);
  ...
  SKILL_CREATE("follow-up riposte", SKILL_FOLLOWUP_RIPOSTE,
               TAR_PHYS);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 31, 75, SPEC_GUARDIAN);
  SKILL_ADD(CLASS_PALADIN, 51, 60);
  SKILL_ADD(CLASS_ANTIPALADIN, 51, 60);

  SKILL_CREATE("critical attack", SKILL_CRITICAL_ATTACK, TAR_PHYS);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 41, 80, SPEC_SWORDSMAN);
  ...
  /* Anti-paladin Specs */
  SPELL_CREATE_MSG("spawn", SPELL_SPAWN, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
  ...
  SPEC_SKILL_ADD(CLASS_ANTIPALADIN, 31, 85, SPEC_DEMONIC);
  SPEC_SKILL_ADD(CLASS_PALADIN, 31, 85, SPEC_CAVALIER);
  ...
  SPEC_SKILL_ADD(CLASS_SORCERER, 31, 100, SPEC_WIZARD);  
  ...
  SPEC_SKILL_ADD(CLASS_ROGUE, 31, 100, SPEC_SHARPSHOOTER);
  ...
  SKILL_CREATE_WITH_MESSAGES("whirlwind", SKILL_WHIRLWIND, TAR_PHYS, "You slow down exhausted.",
  ...
  // Berserker Specs
  SKILL_CREATE_WITH_MESSAGES("war cry", SKILL_WAR_CRY, TAR_PHYS,
                             "Your battle frenzy fades...", NULL);
  SKILL_ADD(CLASS_BERSERKER, 26, 100);

  SKILL_CREATE("mangle", SKILL_MANGLE, TAR_PHYS);
  SPEC_SKILL_ADD(CLASS_BERSERKER, 51, 90, SPEC_MAULER);
  ...
  SPEC_SKILL_ADD(CLASS_BERSERKER, 51, 90, SPEC_RAGELORD);
  ...
  SPELL_CREATE("create greater dracolich", SPELL_CREATE_GREATER_DRACOLICH, PULSE_SPELLCAST * 10,
                TAR_OBJ_ROOM | TAR_NOCOMBAT, spell_create_greater_dracolich);
  SPELL_ADD(CLASS_NECROMANCER, 12);

  SPELL_CREATE("call avatar", SPELL_CALL_AVATAR, PULSE_SPELLCAST * 10, TAR_OBJ_ROOM | TAR_NOCOMBAT,
                spell_call_avatar);
  SPELL_ADD(CLASS_THEURGIST, 12);
  ...
  SPELL_CREATE_MSG("regeneration", SPELL_REGENERATION, PULSE_SPELLCAST * 3 / 2,
                    TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_regeneration, "You stop regenerating.");
  ...
  SPELL_CREATE_MSG("storm shield", SPELL_STORMSHIELD, PULSE_SPELLCAST * 5, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("accelerated healing", SPELL_ACCEL_HEALING, PULSE_SPELLCAST * (3/2), TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("armor", SPELL_ARMOR, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                   spell_armor, "You feel less &+Wprotected&n.");
  SPELL_ADD(CLASS_CLERIC, 1);
  SPELL_ADD(CLASS_PALADIN, 3);
  SPELL_ADD(CLASS_ANTIPALADIN, 3);

  SPELL_CREATE_MSG("virtue", SPELL_VIRTUE, PULSE_SPELLCAST, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE("shatter", SPELL_SHATTER, PULSE_SPELLCAST * 1, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_shatter);
  SPEC_SPELL_ADD(CLASS_BARD, 9, SPEC_DISHARMONIST);

  SPELL_CREATE("enrage", SPELL_ENRAGE, 1, TAR_CHAR_ROOM + TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_enrage);
  ...
  SPELL_CREATE_MSG("fire aura", SPELL_FIRE_AURA, PULSE_SPELLCAST * 4, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_fire_aura, "&+BYour &+rfiery aura &+Breturns to normal.&N");
  SPEC_SPELL_ADD(CLASS_PSIONICIST, 12, SPEC_PYROKINETIC);

  SPELL_CREATE("ether warp", SPELL_ETHER_WARP, PULSE_SPELLCAST * 3, TAR_CHAR_WORLD | TAR_NOCOMBAT, 
               spell_ether_warp);
  SPELL_ADD(CLASS_PSIONICIST, 9);
  SPELL_ADD(CLASS_MINDFLAYER, 7);

  SPELL_CREATE("ethereal rift", SPELL_ETHEREAL_RIFT, PULSE_SPELLCAST * 2, TAR_CHAR_WORLD | TAR_NOCOMBAT, 
               spell_ethereal_rift);
  ...
  SPELL_CREATE("thought beacon", SPELL_THOUGHT_BEACON, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_thought_beacon);
  SPEC_SPELL_ADD(CLASS_PSIONICIST, 12, SPEC_PSYCHEPORTER);

  SPELL_CREATE("radial navigation", SPELL_RADIAL_NAV, PULSE_SPELLCAST * 2, TAR_IGNORE | TAR_NOCOMBAT,
               spell_radial_navigation);
  SPELL_ADD(CLASS_MINDFLAYER, 8);

  SPELL_CREATE_MSG("molecular control", SPELL_MOLECULAR_CONTROL, PULSE_SPELLCAST, TAR_SELF_ONLY | TAR_NOCOMBAT,
  ...
  SPELL_CREATE_MSG("adrenaline control", SPELL_ADRENALINE_CONTROL, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_adrenaline_control, "&+rYour rush of adrenaline runs dry.&n");
  SPELL_ADD(CLASS_PSIONICIST, 1);
  SPELL_ADD(CLASS_MINDFLAYER, 1);

  SPELL_CREATE_MSG("aura sight", SPELL_AURA_SIGHT, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_aura_sight, "&+CThe auras in your vision disappear.&n");
  SPELL_ADD(CLASS_PSIONICIST, 3);
  SPELL_ADD(CLASS_MINDFLAYER, 3);

  SPELL_CREATE_MSG("awe", SPELL_AWE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE("mental anguish", SPELL_MENTAL_ANGUISH, PULSE_SPELLCAST,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_CHAR_RANGE | TAR_AGGRO, spell_mental_anguish);
  SPELL_ADD(CLASS_MINDFLAYER, 8);
  ...
  SPELL_CREATE_MSG("biofeedback", SPELL_BIOFEEDBACK, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_biofeedback, "&+GThe green mist around your body fades...&n");
  SPELL_ADD(CLASS_MINDFLAYER, 7);
  SPELL_ADD(CLASS_PSIONICIST, 11);

  SPELL_CREATE_MSG("displacement", SPELL_DISPLACEMENT, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_displacement, "&+WYour body shifts back into this space-time.&n");
  SPELL_ADD(CLASS_MINDFLAYER, 8);
  SPEC_SPELL_ADD(CLASS_PSIONICIST, 9, SPEC_PSYCHEPORTER);

  SPELL_CREATE("cell adjustment", SPELL_CELL_ADJUSTMENT, PULSE_SPELLCAST, TAR_SELF_ONLY,
...
/* a fun joke! -Tavril */
  ...
  SPELL_CREATE("psionic cloud", SPELL_PSIONIC_CLOUD, PULSE_SPELLCAST, TAR_OFFAREA | TAR_AREA, 
               spell_psionic_cloud);
  SPELL_ADD(CLASS_MINDFLAYER, 10);

  SPELL_CREATE("ectoplasmic form", SPELL_ECTOPLASMIC_FORM, PULSE_SPELLCAST, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("energy containment", SPELL_ENERGY_CONTAINMENT, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_energy_containment, "&+YYou can no longer absorb energy.&n");
  SPELL_ADD(CLASS_PSIONICIST, 4);
  SPELL_ADD(CLASS_MINDFLAYER, 4);

  SPELL_CREATE_MSG("enhance armor", SPELL_ENHANCE_ARMOR, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_enhance_armor, "&+BThe psionic armor around you fades...&n");
  SPELL_ADD(CLASS_MINDFLAYER, 6);

  SPELL_CREATE_MSG("enhance strength", SPELL_ENHANCED_STR, PULSE_SPELLCAST, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_enhanced_strength, "&+CYour muscles return to normal.&n");
  SPELL_ADD(CLASS_PSIONICIST, 2);
  SPELL_ADD(CLASS_MINDFLAYER, 2);

  SPELL_CREATE_MSG("enhance dexterity", SPELL_ENHANCED_DEX, PULSE_SPELLCAST, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_enhanced_dexterity, "&+yYour coordination decreases.&n");
  SPELL_ADD(CLASS_PSIONICIST, 2);
  SPELL_ADD(CLASS_MINDFLAYER, 2);

  SPELL_CREATE_MSG("enhance agility", SPELL_ENHANCED_AGI, PULSE_SPELLCAST, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_enhanced_agility, "&+CYou feel a bit more clumsy.&n");
  SPELL_ADD(CLASS_PSIONICIST, 2);
  SPELL_ADD(CLASS_MINDFLAYER, 2);

  SPELL_CREATE_MSG("enhance constitution", SPELL_ENHANCED_CON, PULSE_SPELLCAST, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_enhanced_constitution, "&+YYour vitality drains away.&n");
  SPELL_ADD(CLASS_PSIONICIST, 2);
  SPELL_ADD(CLASS_MINDFLAYER, 2);

  SPELL_CREATE_MSG("flesh armor", SPELL_FLESH_ARMOR, PULSE_SPELLCAST, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("intellect fortress", SPELL_INTELLECT_FORTRESS, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_intellect_fortress, "&+yYour virtual fortress crumbles.&n");
  SPELL_ADD(CLASS_PSIONICIST, 6);
  SPELL_ADD(CLASS_MINDFLAYER, 5);

  SPELL_CREATE("lend health", SPELL_LEND_HEALTH, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT, 
               spell_lend_health);
  SPELL_ADD(CLASS_PSIONICIST, 3);
  SPELL_ADD(CLASS_MINDFLAYER, 3);

  SPELL_CREATE_MSG("flight", SPELL_POWERCAST_FLY, PULSE_SPELLCAST, TAR_SELF_ONLY,
                   spell_fly, "&+CYour feet slowly descend to the ground.&n");
  SPELL_ADD(CLASS_PSIONICIST, 6);
  SPELL_ADD(CLASS_MINDFLAYER, 5);

  SPELL_CREATE("confuse", SPELL_CONFUSE, PULSE_SPELLCAST * 2, TAR_AREA | TAR_AGGRO, spell_confuse);
  SPELL_ADD(CLASS_MINDFLAYER, 9);
  SPEC_SPELL_ADD(CLASS_PSIONICIST, 12, 1);

  SPELL_CREATE_MSG("mind blank", SPELL_MIND_BLANK, PULSE_SPELLCAST, TAR_SELF_ONLY,
                spell_mind_blank, "&+YYour mind is not as concealed...&N");
  SPELL_ADD(CLASS_MINDFLAYER, 12);
...
/* Removed
  ...
  SPELL_ADD(CLASS_MINDFLAYER, 11);
*/
  SPELL_CREATE("mind travel", SPELL_MIND_TRAVEL, PULSE_SPELLCAST * 4, TAR_SELF_ONLY | TAR_NOCOMBAT, 
               spell_mind_travel);
  SPELL_ADD(CLASS_MINDFLAYER, 11);

  SPELL_CREATE_MSG("tower of iron will", SPELL_TOWER_IRON_WILL, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
                   spell_tower_iron_will, "&+YYou just lost your tower of iron will!&N");
  SPELL_ADD(CLASS_PSIONICIST, 7);
  SPELL_ADD(CLASS_MINDFLAYER, 7);

  SPELL_CREATE("celerity", SPELL_CELERITY, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT, spell_celerity);
//  SPELL_ADD(CLASS_PSIONICIST, 5);
...
/***** End of psionics *****/
  ...
  SPELL_CREATE_MSG("frostbite", SPELL_FROSTBITE, PULSE_SPELLCAST * 11/9,
                   ...
                   spell_frostbite, "You feel less &+Cfrosty&n.");
  ...
  SPELL_CREATE("entropy storm", SPELL_ENTROPY_STORM, PULSE_SPELLCAST * 2, TAR_IGNORE | TAR_AGGRO, 
               spell_entropy_storm);
  SPELL_ADD(CLASS_WARLOCK, 11);

  SPELL_CREATE("chain lightning", SPELL_CHAIN_LIGHTNING, PULSE_SPELLCAST * 2,
               TAR_AREA + TAR_OFFAREA | TAR_AGGRO, spell_chain_lightning);
  SPELL_ADD(CLASS_SORCERER, 10);

  SPELL_CREATE("lightning ring", SPELL_RING_LIGHTNING, (int) (PULSE_SPELLCAST * 2.5),
               TAR_AREA + TAR_OFFAREA | TAR_AGGRO, spell_ring_lightning);
  ...
  SPELL_CREATE("dark compact", SPELL_DARK_COMPACT, PULSE_SPELLCAST, TAR_SELF_ONLY, 
               spell_dark_compact);
...
//  SPELL_ADD(CLASS_THEURGIST, 4);
  ...
  SPELL_CREATE("harm", SPELL_HARM, PULSE_SPELLCAST * (4/3), TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_harm);
  ...
  SPELL_CREATE("taint", SPELL_TAINT, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_taint);
  SPEC_SPELL_ADD(CLASS_NECROMANCER, 9, SPEC_NECROLYTE);
//  SPEC_SPELL_ADD(CLASS_THEURGIST, 9, SPEC_MEDIUM);
//  SPELL_ADD(CLASS_ANTIPALADIN, 11);
//  SPELL_ADD(CLASS_CLERIC, 10);

  SPELL_CREATE("firestorm", SPELL_FIRESTORM, PULSE_SPELLCAST * 3,
               TAR_AREA | TAR_OFFAREA | TAR_FIGHT_VICT | TAR_AGGRO, spell_firestorm);
  SPEC_SPELL_ADD(CLASS_DRUID, 9, SPEC_STORM);
  SPEC_SPELL_ADD(CLASS_REAVER, 10, SPEC_FLAME_REAVER);

  SPELL_CREATE("flame strike", SPELL_FLAMESTRIKE, PULSE_SPELLCAST * 2,
  ...
  SPELL_CREATE_MSG("minor globe of invulnerability", SPELL_MINOR_GLOBE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("deflect", SPELL_DEFLECT, PULSE_SPELLCAST * 4, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("infravision", SPELL_INFRAVISION, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_NOCOMBAT,
  ...
  SPELL_CREATE_MSG("mage flame", SPELL_MAGE_FLAME, (PULSE_SPELLCAST * 3) / 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_mage_flame, "Your mage flame slowly fades into nothingness.");
  SPELL_ADD(CLASS_CONJURER, 3);
  
  SPELL_CREATE_MSG("holy light", SPELL_HOLY_LIGHT, (PULSE_SPELLCAST * 3) / 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_holy_light, "Your holy light slowly fades into nothingness.");
  SPELL_ADD(CLASS_THEURGIST, 9);

  SPELL_CREATE("globe of darkness", SPELL_GLOBE_OF_DARKNESS, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
               spell_globe_of_darkness);
  SPELL_ADD(CLASS_NECROMANCER, 9);

  SPELL_CREATE("flame blade", SPELL_FLAME_BLADE, PULSE_SPELLCAST * 2, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_flame_blade);
  ...
  SPELL_CREATE("minor creation", SPELL_MINOR_CREATION, PULSE_SPELLCAST, TAR_IGNORE | TAR_NOCOMBAT, 
               cast_minor_creation);
  SPELL_ADD(CLASS_CONJURER, 1);
  SPELL_ADD(CLASS_SORCERER, 1);
  SPELL_ADD(CLASS_BARD, 4);
  ...
  SPELL_CREATE("create food", SPELL_CREATE_FOOD, PULSE_SPELLCAST, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_create_food);
  //SPELL_ADD(CLASS_DRUID, 1);
  ...
  SPELL_CREATE("vigorize light", SPELL_VIGORIZE_LIGHT, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM, 
  ...
  SPELL_CREATE("cure blind", SPELL_CURE_BLIND, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM, spell_cure_blind);
  SPELL_ADD(CLASS_CLERIC, 3);
  SPELL_ADD(CLASS_RANGER, 6);
  SPELL_ADD(CLASS_PALADIN, 4);
  SPELL_ADD(CLASS_WARLOCK, 6);

  SPELL_CREATE2("wandering woods", SPELL_WANDERING_WOODS, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT,
                spell_wandering_woods, "The forest falls asleep again as your enchantment fades...",
                "The seemingly endless woods take on a more varied and remarkable appearance.");
  SPELL_ADD(CLASS_DRUID, 9);

  SPELL_CREATE_MSG("aid", SPELL_AID, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_aid, "&+gThe aid of nature leaves you.&n");
  SPELL_ADD(CLASS_DRUID, 2);

  SPELL_CREATE("devitalize", SPELL_DEVITALIZE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM, spell_devitalize);
  SPELL_ADD(CLASS_WARLOCK, 5);

  SPELL_CREATE("vigorize serious", SPELL_VIGORIZE_SERIOUS, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE("vigorize critic", SPELL_VIGORIZE_CRITIC, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("farsee", SPELL_FARSEE, PULSE_SPELLCAST * 5 / 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
  ...
  SPELL_CREATE_MSG("fear", SPELL_FEAR, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
                spell_fear, "You feel less cowardly.");
  SPELL_ADD(CLASS_CLERIC, 6);
  SPELL_ADD(CLASS_ANTIPALADIN, 5);

  SPELL_CREATE("recharger", SPELL_RECHARGER, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM, spell_recharger);
  //SPELL_ADD(CLASS_PSIONICIST, 2);

  SPELL_CREATE("cure light", SPELL_CURE_LIGHT, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_cure_light);
  SPELL_ADD(CLASS_CLERIC, 1);
  SPELL_ADD(CLASS_PALADIN, 3);

  SPELL_CREATE("cure disease", SPELL_CURE_DISEASE, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_cure_disease);
  SPELL_ADD(CLASS_CLERIC, 5);
  ...
  SPELL_CREATE("cure serious", SPELL_CURE_SERIOUS, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM, 
               spell_cure_serious);
  SPELL_ADD(CLASS_CLERIC, 2);
  SPELL_ADD(CLASS_PALADIN, 5);
  ...
  SPELL_CREATE_MSG("vitalize soul", SPELL_VITALIZE_SOUL, PULSE_SPELLCAST * 7 / 3, TAR_CHAR_ROOM,
		   spell_vitalize_undead, "Your enhanced spirit drains away.");
  SPELL_ADD(CLASS_THEURGIST, 6);

  SPELL_CREATE_MSG("vitality", SPELL_VITALITY, PULSE_SPELLCAST * 7 / 3, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("divine fury", SPELL_DIVINE_FURY, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("detect evil", SPELL_DETECT_EVIL, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
                   spell_detect_evil, "You feel the &+rcrimson &nin your vision fade.");
  SPELL_ADD(CLASS_CLERIC, 1);
  SPELL_ADD(CLASS_DRUID, 2);
  SPELL_ADD(CLASS_RANGER, 3);

  SPELL_CREATE_MSG("detect good", SPELL_DETECT_GOOD, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
                   spell_detect_good, "You feel the &+Ygold &nin your vision fade.");
  SPELL_ADD(CLASS_CLERIC, 1);
  SPELL_ADD(CLASS_DRUID, 2);
  SPELL_ADD(CLASS_RANGER, 3);

  SPELL_CREATE_MSG("detect magic", SPELL_DETECT_MAGIC, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
  ...
  SPELL_CREATE("plane shift", SPELL_PLANE_SHIFT, PULSE_SPELLCAST * 2, TAR_IGNORE | TAR_NOCOMBAT, 
               cast_plane_shift);
  SPELL_ADD(CLASS_CLERIC, 9);
  SPELL_ADD(CLASS_WARLOCK, 9);
  ...
  SPELL_CREATE("nether gate", SPELL_NETHER_GATE, PULSE_SPELLCAST * 3, TAR_IGNORE | TAR_NOCOMBAT, 
               cast_nether_gate);
  SPELL_ADD(CLASS_WARLOCK, 10);

  SPELL_CREATE_MSG("sense holiness", SPELL_SENSE_HOLINESS, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                   spell_sense_holiness, "&+wYou can no longer sense holiness.");
  SPELL_ADD(CLASS_WARLOCK, 1);

  SPELL_CREATE_MSG("negative energy barrier", SPELL_NEG_ENERGY_BARRIER, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
  ...
  SPELL_CREATE("resurrect", SPELL_RESURRECT, PULSE_SPELLCAST * 10, TAR_OBJ_ROOM | TAR_NOCOMBAT | TAR_SPIRIT, 
               spell_resurrect);
  SPELL_ADD(CLASS_CLERIC, 9);
  SPEC_SPELL_ADD(CLASS_SHAMAN, 11, SPEC_SPIRITUALIST);
  
  SPELL_CREATE("hand of death", SPELL_HANDOFDEATH, PULSE_SPELLCAST * 10, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_resurrect);
  SPELL_ADD(CLASS_WARLOCK, 10);

  SPELL_CREATE_MSG("death blessing", SPELL_DEATH_BLESSING, PULSE_SPELLCAST * 7 / 3, TAR_IGNORE,
                spell_death_blessing, "You feel less vitalized.");
  SPELL_ADD(CLASS_WARLOCK, 12);

  SPELL_CREATE("preserve", SPELL_PRESERVE, PULSE_SPELLCAST * 2, TAR_OBJ_ROOM | TAR_NOCOMBAT, spell_preserve);
  SPELL_ADD(CLASS_CLERIC, 2);
  SPELL_ADD(CLASS_NECROMANCER, 1);
  SPELL_ADD(CLASS_THEURGIST, 1);
  SPELL_ADD(CLASS_DRUID, 2);
  ...
  SPELL_CREATE("mass preserve", SPELL_MASS_PRESERVE, PULSE_SPELLCAST * 3, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_mass_preserve);
  SPELL_ADD(CLASS_CLERIC, 11);

  SPELL_CREATE_MSG("mass invisibility", SPELL_MASS_INVIS, PULSE_SPELLCAST * 3,  TAR_IGNORE | TAR_NOCOMBAT,
  ...
  SPELL_CREATE("knock", SPELL_KNOCK, PULSE_SPELLCAST * 3, TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_knock);
  SPELL_ADD(CLASS_NECROMANCER, 7);
  SPELL_ADD(CLASS_THEURGIST, 7);

  SPELL_CREATE_MSG("lifelust", SPELL_LIFELUST, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("improved invisibility", SPELL_INVIS_MAJOR, PULSE_SPELLCAST * 4 / 3,
                   TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_OBJ_EQUIP | TAR_NOCOMBAT, 
                   spell_improved_invisibility, "You slowly fade back into visibility.");
  SPELL_ADD(CLASS_NECROMANCER, 8);
  SPELL_ADD(CLASS_SORCERER, 7);
  SPELL_ADD(CLASS_CONJURER, 8);
  SPELL_ADD(CLASS_THEURGIST, 8);
  ...
  SPELL_CREATE_MSG("poison", SPELL_POISON, PULSE_SPELLCAST * 2,
                   TAR_CHAR_ROOM | TAR_SELF_NONO | TAR_OBJ_INV | TAR_OBJ_EQUIP | TAR_FIGHT_VICT | TAR_AGGRO, 
                   spell_poison, "You feel the &+gpoison &nin your &+Rblood &ndissipate.");
  SPELL_ADD(CLASS_DRUID, 4);
  SPEC_SPELL_ADD(CLASS_CLERIC, 4, SPEC_ZEALOT);

  SPELL_CREATE_MSG("protection from evil", SPELL_PROTECT_FROM_EVIL, PULSE_SPELLCAST, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("protection from good", SPELL_PROTECT_FROM_GOOD, PULSE_SPELLCAST, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE("slashing darkness", SPELL_SLASHING_DARKNESS, PULSE_SPELLCAST * 2/3,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_CHAR_RANGE | TAR_RANGE2 | TAR_AGGRO, 
               spell_slashing_darkness);
  SPELL_ADD(CLASS_NECROMANCER, 1);
  SPELL_ADD(CLASS_CABALIST, 1);

  SPELL_CREATE("raise spectre", SPELL_RAISE_SPECTRE, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_raise_spectre);
  SPELL_ADD(CLASS_NECROMANCER, 5);
  ...
  SPELL_CREATE("call asura", SPELL_CALL_ASURA, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_call_asura);
  SPELL_ADD(CLASS_THEURGIST, 5);

  SPELL_CREATE("call bralani", SPELL_CALL_BRALANI, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_call_bralani);
  SPELL_ADD(CLASS_THEURGIST, 7);

  SPELL_CREATE("call knight", SPELL_CALL_KNIGHT, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_call_knight);
  SPELL_ADD(CLASS_THEURGIST, 9);

  SPELL_CREATE("call liberator", SPELL_CALL_LIBERATOR, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_call_liberator);
  SPELL_ADD(CLASS_THEURGIST, 10);

  SPELL_CREATE("animate dead", SPELL_ANIMATE_DEAD, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_animate_dead);
  SPELL_ADD(CLASS_NECROMANCER, 3);
  
  SPELL_CREATE("call archon", SPELL_CALL_ARCHON, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT, 
               spell_call_archon);
  ...
  SPELL_CREATE("vortex of fear", SPELL_VORTEX_OF_FEAR, PULSE_SPELLCAST * 3, 
               TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_vortex_of_fear);
  SPELL_ADD(CLASS_ANTIPALADIN, 10);

  SPELL_CREATE_MSG("righteous aura", SPELL_RIGHTEOUS_AURA, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
                   spell_righteous_aura, "&+WThe power of your god leaves your body, and you feel weaker.&n");
  SPELL_ADD(CLASS_PALADIN, 12);

  SPELL_CREATE_MSG("bleak foeman", SPELL_BLEAK_FOEMAN, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY, spell_bleak_foeman, 
                   "&+LAs the evil exits your body, you suddenly feel less compelled to strike down your enemies.");
  ...
  SPELL_CREATE_MSG("group stone skin", SPELL_GROUP_STONE_SKIN, PULSE_SPELLCAST * 5, TAR_IGNORE,
                   spell_group_stone_skin, "You fell your skin soften and return to normal.");
  SPELL_ADD(CLASS_CONJURER, 10);

  SPELL_CREATE_MSG("sleep", SPELL_SLEEP, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
  ...
  SPELL_CREATE("dispel magic", SPELL_DISPEL_MAGIC, PULSE_SPELLCAST * 2,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_WALL | TAR_AGGRO, 
               spell_dispel_magic);
  SPELL_ADD(CLASS_CONJURER, 3);
  SPELL_ADD(CLASS_SORCERER, 4);
  SPELL_ADD(CLASS_NECROMANCER, 4);
  ...
  SPELL_CREATE_MSG("strength", SPELL_STRENGTH, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_NOCOMBAT,
...
//  SPELL_ADD(CLASS_BARD, 3);
  ...
  SPELL_CREATE_MSG("dazzle", SPELL_DAZZLE, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
                   spell_dazzle, "You feel the energy seep out of you.");
  SPELL_ADD(CLASS_RANGER, 10);

  SPELL_CREATE_MSG("blur", SPELL_BLUR, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("stornogs metamagic shroud", SPELL_STORNOGS_LOWERED_RES, PULSE_SPELLCAST * 3,
                   TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
                   spell_stornogs_lowered_magical_res, "&+WYou feel your innate magical resistance return.");
  SPELL_ADD(CLASS_CONJURER, 7);

  SPELL_CREATE2("stornogs shimmering starshell", SPELL_STARSHELL, (PULSE_SPELLCAST * 3) / 2, TAR_IGNORE,
                spell_starshell, "&+WThe &+Yblazing&N&+W shell dissipates.",
                "&+WThe &+Yblazing&N&+W shell dissipates.");
  SPELL_ADD(CLASS_CONJURER, 10);

  SPELL_CREATE("stornogs spheres", SPELL_STORNOGS_SPHERES, PULSE_SPELLCAST * 4, TAR_SELF_ONLY,
  ...
  SPELL_CREATE("cloak of fear", SPELL_CLOAK_OF_FEAR, PULSE_SPELLCAST, TAR_IGNORE | TAR_AGGRO, 
               spell_cloak_of_fear);
  SPELL_ADD(CLASS_NECROMANCER, 10);
  SPELL_ADD(CLASS_THEURGIST, 10);
  ...
  SPELL_CREATE_MSG("haste", SPELL_HASTE, (PULSE_SPELLCAST * 3) / 2, TAR_CHAR_ROOM, spell_haste,
  ...
  SPELL_CREATE("group recall", SPELL_GROUP_RECALL, PULSE_SPELLCAST * 6, TAR_IGNORE, spell_group_recall);
  SPELL_ADD(CLASS_CLERIC, 11);
  ...
  SPELL_CREATE_MSG("sense follower", SPELL_SENSE_FOLLOWER, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                spell_sense_follower, "You no longer seem to sense your followers.");
  SPELL_ADD(CLASS_NECROMANCER, 3);
  SPELL_ADD(CLASS_CONJURER, 5);
  SPELL_ADD(CLASS_THEURGIST, 3);

  SPELL_CREATE_MSG("sense life", SPELL_SENSE_LIFE, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
  ...
  SPELL_CREATE2("consecrate land", SPELL_CONSECRATE_LAND, PULSE_SPELLCAST * 3, TAR_IGNORE | TAR_SPIRIT,
  ...
  SPELL_CREATE_MSG("protection from fire", SPELL_PROTECT_FROM_FIRE, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                   spell_protection_from_fire, "You no longer feel safe from &+rfire&n.");
  SPELL_ADD(CLASS_CLERIC, 2);
  SPELL_ADD(CLASS_DRUID, 2);
  SPELL_ADD(CLASS_WARLOCK, 3);

  SPELL_CREATE_MSG("protection from cold", SPELL_PROTECT_FROM_COLD, PULSE_SPELLCAST, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE("animal friendship", SPELL_ANIMAL_FRIENDSHIP, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
               spell_animal_friendship);
  SPELL_ADD(CLASS_DRUID, 1);

  SPELL_CREATE_MSG("protection from gas", SPELL_PROTECT_FROM_GAS, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                   spell_protection_from_gas, "You no longer feel safe from &+gpoison gas&n.");
  SPELL_ADD(CLASS_CLERIC, 4);
  SPELL_ADD(CLASS_DRUID, 3);

  SPELL_CREATE_MSG("protection from acid", SPELL_PROTECT_FROM_ACID, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                spell_protection_from_acid, "You no longer feel safe from &+Gacid&n.");
  SPELL_ADD(CLASS_CLERIC, 4);
  SPELL_ADD(CLASS_DRUID, 3);

  SPELL_CREATE_MSG("protection from lightning", SPELL_PROTECT_FROM_LIGHTNING, PULSE_SPELLCAST, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("fly", SPELL_FLY, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_NOCOMBAT,
  ...
  SPELL_CREATE("istr", SPELL_PERM_INCREASE_STR, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_str);

  SPELL_CREATE("iagi", SPELL_PERM_INCREASE_AGI, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_agi);

  SPELL_CREATE("idex", SPELL_PERM_INCREASE_DEX, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_dex);

  SPELL_CREATE("icon", SPELL_PERM_INCREASE_CON, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_con);

  SPELL_CREATE("iluck", SPELL_PERM_INCREASE_LUCK, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_luck);

  SPELL_CREATE("ipow", SPELL_PERM_INCREASE_POW, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_pow);

  SPELL_CREATE("iint", SPELL_PERM_INCREASE_INT, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_int);

  SPELL_CREATE("iwis", SPELL_PERM_INCREASE_WIS, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
               spell_perm_increase_wis);

  SPELL_CREATE("icha", SPELL_PERM_INCREASE_CHA, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
  ...
  SPELL_CREATE_MSG("guardian spirits", SPELL_GUARDIAN_SPIRITS, PULSE_SPELLCAST * 12,
  ...
  SPELL_CREATE("faerie fog", SPELL_FAERIE_FOG, PULSE_SPELLCAST * 2, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_faerie_fog);
  SPELL_ADD(CLASS_DRUID, 3);
...
//  SPELL_ADD(CLASS_THEURGIST, 11);
  ...
  SPELL_CREATE("sunray", SPELL_SUNRAY, PULSE_SPELLCAST * 2, TAR_FIGHT_VICT | TAR_CHAR_ROOM | TAR_AGGRO,
               spell_sunray);
  SPELL_ADD(CLASS_DRUID, 9);

  SPELL_CREATE_MSG("feeblemind", SPELL_FEEBLEMIND, PULSE_SPELLCAST * 2, TAR_FIGHT_VICT | TAR_CHAR_ROOM | TAR_AGGRO,
  ...
  SPELL_CREATE_MSG("silence", SPELL_SILENCE, PULSE_SPELLCAST * 2, TAR_FIGHT_VICT | TAR_CHAR_ROOM | TAR_AGGRO,
                   spell_silence, "You are able to speak again.");
  SPELL_ADD(CLASS_CLERIC, 8);

  SPELL_CREATE("turn undead", SPELL_TURN_UNDEAD, PULSE_SPELLCAST, TAR_IGNORE, spell_turn_undead);
  SPELL_ADD(CLASS_CLERIC, 1);
  SPELL_ADD(CLASS_PALADIN, 5);
  SPELL_ADD(CLASS_AVENGER, 8);

  SPELL_CREATE("command undead", SPELL_COMMAND_UNDEAD, 0, TAR_FIGHT_VICT | TAR_CHAR_ROOM, spell_command_undead);
  // SPELL_ADD(CLASS_NECROMANCER, 5);
  // SPELL_ADD(CLASS_ANTIPALADIN, 10);

  SPELL_CREATE("web", SPELL_WEB, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT, cast_web);
  SPELL_ADD(CLASS_CONJURER, 7);

  SPELL_CREATE("wall of flames", SPELL_WALL_OF_FLAMES, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT, 
               cast_wall_of_flames);
  SPELL_ADD(CLASS_CONJURER, 7);
  ...
  SPELL_CREATE("wall of iron", SPELL_WALL_OF_IRON, PULSE_SPELLCAST * 5, TAR_IGNORE | TAR_NOCOMBAT, 
               cast_wall_of_iron);
  SPELL_ADD(CLASS_CONJURER, 6);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 10, SPEC_COSMOMANCER);

  SPELL_CREATE("wall of force", SPELL_WALL_OF_FORCE, PULSE_SPELLCAST * 5, TAR_IGNORE | TAR_NOCOMBAT, 
               cast_wall_of_force);
  SPELL_ADD(CLASS_CONJURER, 9);
  ...
  SPELL_CREATE_MSG("slow poison", SPELL_SLOW_POISON, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                spell_slow_poison, "Your enhanced resistance to &+gpoison&n fades...&n");
  SPELL_ADD(CLASS_CLERIC, 2);

  SPELL_CREATE_MSG("coldshield", SPELL_COLDSHIELD, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("charm animal", SPELL_CHARM_ANIMAL, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_SELF_NONO,
                spell_charm_animal, "You regain your senses.");

  SPELL_CREATE_MSG("soulshield", SPELL_SOULSHIELD, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
                spell_soulshield, "&+WThe spiritual aura about your body fades&n.");
  SPELL_ADD(CLASS_CLERIC, 5);
  SPELL_ADD(CLASS_PALADIN, 7);
  SPELL_ADD(CLASS_ANTIPALADIN, 7);
  
  SPELL_CREATE_MSG("holy aura", SPELL_HOLY_AURA, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
                spell_soulshield, "&+WThe spiritual aura about your body fades&n.");
  SPELL_ADD(CLASS_THEURGIST, 7);

  SPELL_CREATE_MSG("dharma", SPELL_HOLY_DHARMA, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
                   spell_holy_dharma, "&+cThe &+Cdivine &+cinspiration departs from your soul.&n");
  SPELL_ADD(CLASS_PALADIN, 9);

  SPELL_CREATE_MSG("holy sacrifice", SPELL_HOLY_SACRIFICE, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("true seeing", SPELL_TRUE_SEEING, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_true_seeing, "&+MYour vision &n&+mdulls.&n");
  SPELL_ADD(CLASS_CLERIC, 11);

  SPELL_CREATE_MSG("shadow vision", SPELL_SHADOW_VISION, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                spell_shadow_vision, "&+MYour vision &n&+mdulls.&n");
  SPELL_ADD(CLASS_WARLOCK, 11);

  SPELL_CREATE_MSG("animal vision", SPELL_ANIMAL_VISION, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
  ...
  SPELL_CREATE_MSG("natures blessing", SPELL_NATURES_BLESSING, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
                   spell_natures_blessing, "The blessing of nature has left you.");
  SPELL_ADD(CLASS_DRUID, 10);
  ...
  SPELL_CREATE_MSG("wolfspeed", SPELL_WOLFSPEED, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_ANIMAL,
...
//  SPELL_ADD(CLASS_SHAMAN, 12);
  ...
  SPELL_CREATE("beastform", SPELL_BEASTFORM, PULSE_SPELLCAST * 3, TAR_SELF_ONLY | TAR_NOCOMBAT | TAR_ANIMAL,
               spell_beastform);
//  SPELL_ADD(CLASS_SHAMAN, 5);
  ...
  SPELL_CREATE_MSG("elemental affinity", SPELL_ELEM_AFFINITY, PULSE_SPELLCAST * 4, 
                   ...
                   "&+BA slight &+bchill &+Bruns through you as you feel &+gnature's essence &+Bleave your body.&N");
  SPEC_SPELL_ADD(CLASS_SHAMAN, 7, SPEC_ELEMENTALIST);

  SPELL_CREATE("cascading elemental beam", SPELL_CASCADING_ELEMENTAL_BEAM, PULSE_SPELLCAST * 3 / 2,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_ELEMENTAL,
  ...
  SPELL_CREATE_MSG("cold ward", SPELL_COLD_WARD, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_ELEMENTAL,
                   spell_cold_ward, "&+cYou no longer feel protected from the cold.");
  SPELL_ADD(CLASS_SHAMAN, 3);

  SPELL_CREATE_MSG("fire ward", SPELL_FIRE_WARD, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_ELEMENTAL,
  ...
  SPELL_CREATE("arieks shattering iceball", SPELL_ARIEKS_SHATTERING_ICEBALL, PULSE_SPELLCAST * 3 / 2,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_ELEMENTAL,
  ...
  SPELL_CREATE("tormenting spirits", SPELL_TORMENT_SPIRITS, PULSE_SPELLCAST * 5 / 2,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_SPIRIT,
               spell_torment_spirits);
  SPEC_SPELL_ADD(CLASS_SHAMAN, 8, SPEC_SPIRITUALIST);

  SPELL_CREATE_MSG("indomitability", SPELL_INDOMITABILITY, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_SPIRIT,
                   spell_indomitability, "&+BYour spiritual indomitability &+bfades...&N");
  SPEC_SPELL_ADD(CLASS_SHAMAN, 7, SPEC_SPIRITUALIST);

  SPELL_CREATE_MSG("spirit armor", SPELL_SPIRIT_ARMOR, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT,
                   spell_spirit_armor, "&+mYou feel less protected.");
  SPELL_ADD(CLASS_SHAMAN, 3);

  SPELL_CREATE("transfer wellness", SPELL_TRANSFER_WELLNESS, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT,
               spell_transfer_wellness);
  SPELL_ADD(CLASS_SHAMAN, 1);

  SPELL_CREATE("sustenance", SPELL_SUSTENANCE, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_SPIRIT,
               spell_sustenance);
  //SPELL_ADD(CLASS_SHAMAN, 2);
  ...
  SPELL_CREATE("purify spirit", SPELL_PURIFY_SPIRIT, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT,
               spell_purify_spirit);
  SPELL_ADD(CLASS_SHAMAN, 3);
  SPELL_ADD(CLASS_ETHERMANCER, 2);

  SPELL_CREATE("lesser mending", SPELL_LESSER_MENDING, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT, 
               spell_lesser_mending);
  SPELL_ADD(CLASS_SHAMAN, 2);

  SPELL_CREATE("mending", SPELL_MENDING, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT, spell_mending);
  SPELL_ADD(CLASS_SHAMAN, 4);
  ...
  SPELL_CREATE_MSG("sense spirit", SPELL_SENSE_SPIRIT, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_SPIRIT,
                   spell_sense_spirit, "&+LYour spirit sense fades...");
  SPELL_ADD(CLASS_SHAMAN, 5);

  SPELL_CREATE_MSG("malison", SPELL_MALISON, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_SPIRIT,
                   spell_malison, "You feel less cursed.");
  SPELL_ADD(CLASS_SHAMAN, 5);

  SPELL_CREATE("wellness", SPELL_WELLNESS, PULSE_SPELLCAST, TAR_IGNORE | TAR_SPIRIT, spell_wellness);
  SPELL_ADD(CLASS_SHAMAN, 6);

  SPELL_CREATE("greater mending", SPELL_GREATER_MENDING, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT,
  ...
  SPELL_CREATE("reveal true form", SPELL_REVEAL_TRUE_FORM, PULSE_SPELLCAST, TAR_IGNORE | TAR_NOCOMBAT | TAR_SPIRIT,
  ...
  SPELL_CREATE_MSG("greater spirit ward", SPELL_GREATER_SPIRIT_WARD, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_SPIRIT,
  ...
  SPELL_CREATE("restoration", SPELL_RESTORATION, PULSE_SPELLCAST * 5/2, TAR_CHAR_ROOM | TAR_SPIRIT,
               spell_restoration);
  SPEC_SPELL_ADD(CLASS_SHAMAN, 10, SPEC_SPIRITUALIST);

  SPELL_CREATE("summon spirit", SPELL_SUMMON_SPIRIT, PULSE_SPELLCAST, TAR_IGNORE | TAR_NOCOMBAT | TAR_SPIRIT,
               spell_summon_spirit);
  SPELL_ADD(CLASS_SHAMAN, 7);
  ...
  /* Reaver spells */
  SPELL_CREATE_MSG("baladors protection", SPELL_BALADORS_PROTECTION, PULSE_SPELLCAST,  TAR_SELF_ONLY,
                   spell_baladors_protection, "You feel Balador's protection leave you.");
  SPELL_ADD(CLASS_REAVER, 5);

  SPELL_CREATE_MSG("ferrix precision", SPELL_FERRIX_PRECISION, PULSE_SPELLCAST, TAR_SELF_ONLY,
  ...
  SPELL_CREATE_MSG("cegilunes searing blade", SPELL_CEGILUNE_BLADE, PULSE_SPELLCAST * 4 / 3,
                   TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_cegilunes_searing_blade, "&+RYou feel weaker as &+rCegilune's &+Ressence leaves you.&n");
  SPEC_SPELL_ADD(CLASS_REAVER, 12, SPEC_FLAME_REAVER);

  SPELL_CREATE_MSG("kanchelsis fury", SPELL_KANCHELSIS_FURY, (PULSE_SPELLCAST * 3) / 2, TAR_SELF_ONLY,
                   spell_kanchelsis_fury, "&+LThe world speeds up around you.&n");
  SPELL_ADD(CLASS_REAVER, 8);

  SPELL_CREATE("blood alliance", SPELL_BLOOD_ALLIANCE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_SELF_NONO,
               spell_blood_alliance);

  SPELL_CREATE_MSG("ilienzes flaming sword", SPELL_ILIENZES_FLAME_SWORD, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
                   spell_ilienzes_flame_sword, "&+rYou feel weaker as &+RIlienze's &+ressence leaves you.&n");
  SPEC_SPELL_ADD(CLASS_REAVER, 11, SPEC_FLAME_REAVER);

  SPELL_CREATE_MSG("thryms icerazor", SPELL_THRYMS_ICERAZOR, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
                   spell_thryms_icerazor, "&+CYou feel weaker as &+LThrym's &+cessence &+Cleaves you.&n");
  ...
  SPELL_CREATE_MSG("lliendils stormshock", SPELL_LLIENDILS_STORMSHOCK, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
                   spell_lliendils_stormshock, "&+BThe essence of &+LLliendil &+Bleaves your weapon.&n");
  SPEC_SPELL_ADD(CLASS_REAVER, 11, SPEC_SHOCK_REAVER);

  SPELL_CREATE_MSG("umberlees fury", SPELL_UMBERLEES_FURY, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY | TAR_NOCOMBAT,
                   spell_umberlees_fury, "&+BThe essence of &+bUmberlee &+bleaves your weapon.&n");
  SPEC_SPELL_ADD(CLASS_REAVER, 12, SPEC_SHOCK_REAVER);

  SPELL_CREATE_MSG("lightning shield", SPELL_LIGHTNINGSHIELD, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
...
/* Illusionists */
  ...
  SPELL_CREATE_MSG("phantom armor", SPELL_PHANTOM_ARMOR, PULSE_SPELLCAST, TAR_SELF_ONLY,
  ...
  SPELL_CREATE("insects", SPELL_INSECTS, PULSE_SPELLCAST * 4 / 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_insects);
  SPELL_ADD(CLASS_ILLUSIONIST, 4);

  SPELL_CREATE("illusionary wall", SPELL_ILLUSIONARY_WALL, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT,
               spell_illusionary_wall);
  SPELL_ADD(CLASS_ILLUSIONIST, 5);

  SPELL_CREATE("boulder", SPELL_BOULDER, PULSE_SPELLCAST * 5 / 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_boulder);
  SPELL_ADD(CLASS_ILLUSIONIST, 5);
  ...
  SPELL_CREATE("stunning visions", SPELL_STUNNING_VISIONS, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
               spell_stunning_visions);
  SPELL_ADD(CLASS_ILLUSIONIST, 8);

  SPELL_CREATE("reflection", SPELL_REFLECTION, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_reflection);
  ...
  SPELL_CREATE_MSG("shadow shield", SPELL_SHADOW_SHIELD, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
                   spell_shadow_shield, 
                   "&+wYou feel the &+Ls&+Ww&+Li&+Wr&+Ll&+Wi&+Ln&+Wg &+wshadows around your body dissipate.&n");
  SPELL_ADD(CLASS_ILLUSIONIST, 8);

  SPELL_CREATE_MSG("vanish", SPELL_VANISH, PULSE_SPELLCAST, TAR_SELF_ONLY | TAR_NOCOMBAT, 
                   spell_vanish, "&+WYou snap back into visibility.&n");
  SPELL_ADD(CLASS_ILLUSIONIST, 8);

  SPELL_CREATE("hammer", SPELL_HAMMER, PULSE_SPELLCAST * 5 / 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
               spell_hammer);
  SPELL_ADD(CLASS_ILLUSIONIST, 7);

  SPELL_CREATE_MSG("detect illusion", SPELL_DETECT_ILLUSION, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
                   spell_detect_illusion, "Unseen things and illusion vanish from your sight.");
  SPELL_ADD(CLASS_ILLUSIONIST, 9);

  SPELL_CREATE("dream travel", SPELL_SHADOW_RIFT, PULSE_SPELLCAST * 4, TAR_CHAR_WORLD | TAR_NOCOMBAT, 
               spell_shadow_rift);
  SPELL_ADD(CLASS_ILLUSIONIST, 9);

  SPELL_CREATE("clone form", SPELL_CLONE_FORM, PULSE_SPELLCAST * 4, TAR_CHAR_ROOM | TAR_NOCOMBAT, 
               spell_clone_form);
  SPELL_ADD(CLASS_ILLUSIONIST, 10);

  SPELL_CREATE("imprisonment", SPELL_IMPRISON, PULSE_SPELLCAST * 3,
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_NOCOMBAT | TAR_AGGRO, spell_imprison);
  SPELL_ADD(CLASS_ILLUSIONIST, 10);

  SPELL_CREATE_MSG("nonexistence", SPELL_NONEXISTENCE, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT,
                   spell_nonexistence, "You slowly fade back into visibility.");
  SPELL_ADD(CLASS_ILLUSIONIST, 11);

  SPELL_CREATE("dragon", SPELL_DRAGON, PULSE_SPELLCAST * 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
               spell_dragon);
  SPELL_ADD(CLASS_ILLUSIONIST, 12);

  SPELL_CREATE("titan", SPELL_TITAN, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,  spell_titan);
  SPELL_ADD(CLASS_ILLUSIONIST, 11);

  SPELL_CREATE_MSG("delirium", SPELL_DELIRIUM, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
                   spell_delirium, "&+WYou feel less &+Gconfused.&n");
  SPELL_ADD(CLASS_ILLUSIONIST, 10);

  SPELL_CREATE("flicker", SPELL_FLICKER, PULSE_SPELLCAST * 5, TAR_IGNORE | TAR_NOCOMBAT, spell_flicker);
  SPELL_ADD(CLASS_ILLUSIONIST, 11);

// Ethermancer

  SPELL_CREATE_MSG("vapor armor", SPELL_VAPOR_ARMOR, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_NOCOMBAT,
                   spell_vapor_armor, "&+CYou feel the protection of the winds dissipate.");
  SPELL_ADD(CLASS_ETHERMANCER, 1);

  SPELL_CREATE_MSG("faerie sight", SPELL_FAERIE_SIGHT, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
                spell_faerie_sight, "You feel the &+mtwinkle&n in your eyes fade.");
  SPELL_ADD(CLASS_ETHERMANCER, 4);

  SPELL_CREATE("frost beacon", SPELL_FROST_BEACON, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_frost_beacon);
  SPELL_ADD(CLASS_ETHERMANCER, 6);

  SPELL_CREATE("cold snap", SPELL_COLD_SNAP, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_cold_snap);
  SPELL_ADD(CLASS_ETHERMANCER, 7);
  SPEC_SPELL_ADD(CLASS_REAVER, 8, SPEC_ICE_REAVER);

  SPELL_CREATE_MSG("vapor strike", SPELL_VAPOR_STRIKE, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_NOCOMBAT,
                   spell_vapor_strike, "&+CThe vapors subside, leaving you weak.&n");
  SPELL_ADD(CLASS_ETHERMANCER, 5);

  SPELL_CREATE("wind blade", SPELL_WIND_BLADE, PULSE_SPELLCAST, TAR_IGNORE, spell_wind_blade);
  SPELL_ADD(CLASS_ETHERMANCER, 3);
  ...
  SPELL_CREATE_MSG("ethereal form", SPELL_ETHEREAL_FORM, PULSE_SPELLCAST * 3, TAR_SELF_ONLY | TAR_NOCOMBAT,
  ...
  SPELL_CREATE("forked lightning", SPELL_FORKED_LIGHTNING, PULSE_SPELLCAST * 2, 
               TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
  ...
  SPELL_CREATE("conjure ice", SPELL_CONJURE_ICE, PULSE_SPELLCAST * 5, TAR_IGNORE | TAR_NOCOMBAT, 
               spell_conjure_ice_elemental);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 7, SPEC_FROST_MAGUS);

  SPELL_CREATE_MSG("iceflow armor", SPELL_ICE_ARMOR, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
                   spell_iceflow_armor, "&+CThe &+Bicy&n armor encasing you melts away.&n");
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 8, SPEC_FROST_MAGUS);

  SPELL_CREATE_MSG("negative feedback barrier", SPELL_NEG_ARMOR, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
  ...
  SPELL_CREATE("etheric gust", SPELL_ETHERIC_GUST, PULSE_SPELLCAST, TAR_SELF_ONLY, spell_etheric_gust);
  ...
  SPELL_CREATE2("tempest terrain", SPELL_TEMPEST_TERRAIN, PULSE_SPELLCAST * 6, TAR_IGNORE | TAR_NOCOMBAT,
                spell_tempest_terrain, "The terrain becomes solid once again.",
                "The terrain becomes solid once again.");
  SPELL_ADD(CLASS_ETHERMANCER, 11);

  SPELL_CREATE_MSG("storm empathy", SPELL_STORM_EMPATHY, PULSE_SPELLCAST, TAR_CHAR_ROOM,
                   spell_storm_empathy, "You do not feel as safe from &+Bstorms&n.");
  SPELL_ADD(CLASS_ETHERMANCER, 5);

  SPELL_CREATE("mass fly", SPELL_MASS_FLY, PULSE_SPELLCAST * 7, TAR_IGNORE, spell_mass_fly);
  SPELL_ADD(CLASS_ETHERMANCER, 11);
  ...
  SPELL_CREATE("supernova", SPELL_SUPERNOVA, PULSE_SPELLCAST * 3, TAR_AREA + TAR_OFFAREA | TAR_AGGRO, spell_supernova);
  ...
  SPELL_CREATE("comet", SPELL_COMET, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_comet);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 9, SPEC_COSMOMANCER);

  SPELL_CREATE("ethereal discharge", SPELL_ETHEREAL_DISCHARGE, PULSE_SPELLCAST * 3, TAR_AREA + TAR_OFFAREA | TAR_AGGRO,
               spell_ethereal_discharge);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 8, SPEC_COSMOMANCER);

  SPELL_CREATE_MSG("planetary alignment", SPELL_PLANETARY_ALIGNMENT, PULSE_SPELLCAST * 5, TAR_SELF_ONLY,
                   spell_planetary_alignment,
                   "&+wThe planetary alignment of the &+ycosmos &+wis lost, leaving you weaker.");
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 9, SPEC_COSMOMANCER);

  SPELL_CREATE("cosmic rift", SPELL_COSMIC_RIFT, PULSE_SPELLCAST * 6, TAR_AREA + TAR_OFFAREA | TAR_AGGRO, spell_cosmic_rift);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 12, SPEC_COSMOMANCER);

  // Frost Magus Spec
  SPELL_CREATE("squall", SPELL_TEMPEST, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_AGGRO, spell_tempest);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 11, SPEC_FROST_MAGUS);

  SPELL_CREATE_MSG("windrage", SPELL_WIND_RAGE, PULSE_SPELLCAST * 4, TAR_SELF_ONLY,
                   spell_wind_rage, "&+cYour mind begins to slow as the winds subside.");
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 10, SPEC_FROST_MAGUS);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 10, SPEC_WINDTALKER);
  
  SPELL_CREATE("polar vortex", SPELL_POLAR_VORTEX, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_AGGRO, spell_polar_vortex);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 12, SPEC_FROST_MAGUS);

  // Windtalker Spec
  SPELL_CREATE_MSG("ethereal alliance", SPELL_ETHEREAL_ALLIANCE, PULSE_SPELLCAST * 5, TAR_CHAR_ROOM,
                   spell_ethereal_alliance, "&+cYour alliance has been severed.");
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 11, SPEC_WINDTALKER);

  SPELL_CREATE("greater ethereal recharge", SPELL_GREATER_ETHEREAL, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
               spell_greater_ethereal_recharge);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 10, SPEC_WINDTALKER);

  SPELL_CREATE("razor wind", SPELL_RAZOR_WIND, PULSE_SPELLCAST * 3, TAR_AREA | TAR_AGGRO, spell_razor_wind);
 // SPEC_SPELL_ADD(CLASS_ETHERMANCER, 10, SPEC_WINDTALKER);
  ...
  SPELL_CREATE("doom blade", SPELL_DOOM_BLADE, PULSE_SPELLCAST * 8, TAR_IGNORE | TAR_NOCOMBAT, spell_doom_blade);
  SPEC_SPELL_ADD(CLASS_NECROMANCER, 8, SPEC_REAPER);

  SPELL_CREATE("holy blade", SPELL_HOLY_BLADE, PULSE_SPELLCAST * 8, TAR_IGNORE | TAR_NOCOMBAT, spell_doom_blade);
  ...
  SPELL_CREATE_MSG("atonement", SPELL_ATONEMENT, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
                   spell_atonement, "&+WYou sense that your past sins once again need forgiving.&n");
  SPELL_ADD(CLASS_AVENGER, 7);

  SPELL_CREATE_MSG("celestial aura", SPELL_CELESTIAL_AURA, PULSE_SPELLCAST * 5, TAR_SELF_ONLY,
                   spell_celestial_aura, "&+WYou feel your prayers will go unanswered once again.&n");
  SPELL_ADD(CLASS_AVENGER, 12);

  SPELL_CREATE_MSG("chaos shield", SPELL_CHAOS_SHIELD, PULSE_SPELLCAST * 5, TAR_SELF_ONLY | TAR_NOCOMBAT, 
                   spell_chaos_shield, "&+LThe chaotic aura about you dissipates.&n");
  ...
  SPEC_SKILL_ADD(CLASS_ROGUE, 51, 40, SPEC_SHARPSHOOTER);
  SPEC_SKILL_ADD(CLASS_BARD, 51, 40, SPEC_SCOUNDREL);
...
*/
  ...
  SPEC_SKILL_ADD(CLASS_ROGUE, 46, 80, SPEC_ASSASSIN);
  ...
  SKILL_ADD(CLASS_DREADLORD, 41, 100);
  SKILL_ADD(CLASS_AVENGER, 41, 100);
  ...
  SKILL_CREATE_WITH_MESSAGES("blood scent", SKILL_BLOOD_SCENT, TAR_PHYS,
                             ...
                             "$n seems to sniff the air, futility.");
  SPEC_SKILL_ADD(CLASS_DREADLORD, 36, 100, SPEC_DEATHLORD);
  ...
  SKILL_CREATE("switch opponents", SKILL_SWITCH_OPPONENTS, TAR_PHYS);
  SKILL_ADD(CLASS_WARRIOR, 1, 100);
  SKILL_ADD(CLASS_ANTIPALADIN, 31, 95);
  SKILL_ADD(CLASS_PALADIN, 31, 95);
  ...
  SKILL_ADD(CLASS_RANGER, 36, 100);
  SKILL_ADD(CLASS_MONK, 31, 100);
  SKILL_ADD(CLASS_REAVER, 36, 80);
  ...
  SKILL_CREATE_WITH_MESSAGES("heroism", SKILL_HEROISM, TAR_MENTAL, "You no longer feel like a hero!", NULL);
  SKILL_ADD(CLASS_MONK, 1, 100);
  
  SKILL_CREATE_WITH_MESSAGES("diamond soul", SKILL_DIAMOND_SOUL, TAR_MENTAL, 
  ...
  SKILL_ADD(CLASS_MONK, 26, 100);

  SKILL_CREATE("calm", SKILL_CALM, TAR_PHYS);
  SKILL_ADD(CLASS_MONK, 11, 100);
  ...
  SKILL_CREATE("song of forgetfulness", SONG_FORGETFULNESS, TAR_MENTAL);
  SKILL_ADD(CLASS_BARD, 21, 100);
  ...
  //SPEC_SKILL_ADD(CLASS_ROGUE, 1, 100, SPEC_ASSASSIN); Assassins lose circle and gain flank for wipe 2011
  //                                                    - Jexni 7/11/2011

  SPELL_CREATE("airy water", SPELL_AIRY_WATER, PULSE_SPELLCAST * 4, TAR_IGNORE, spell_airy_water);
  SPELL_ADD(CLASS_CONJURER, 7);
  ...
  SKILL_ADD(CLASS_PALADIN, 36, 70);
  SKILL_ADD(CLASS_ANTIPALADIN, 36, 70);
  SKILL_ADD(CLASS_SORCERER, 1, 100);
  SKILL_ADD(CLASS_WARLOCK, 1, 100);
  SKILL_ADD(CLASS_NECROMANCER, 1, 100);
  SKILL_ADD(CLASS_CABALIST, 1, 100);
  ...
  SKILL_CREATE("clerical spell knowledge", SKILL_SPELL_KNOWLEDGE_CLERICAL, TAR_MENTAL);
  SKILL_ADD(CLASS_PALADIN, 11, 80);
  SKILL_ADD(CLASS_ANTIPALADIN, 11, 80);
  SKILL_ADD(CLASS_CLERIC, 1, 100);
  SKILL_ADD(CLASS_DRUID, 1, 100);
  SKILL_ADD(CLASS_RANGER, 11, 80);
  SKILL_ADD(CLASS_WARLOCK, 1, 80);
  SKILL_ADD(CLASS_ETHERMANCER, 1, 80);
  SKILL_ADD(CLASS_AVENGER, 1, 75);

  SKILL_CREATE("sorcerous spell knowledge", SKILL_SPELL_KNOWLEDGE_MAGICAL, TAR_MENTAL);
  SKILL_ADD(CLASS_ANTIPALADIN, 11, 100);
  SKILL_ADD(CLASS_SORCERER, 1, 100);
  SKILL_ADD(CLASS_WARLOCK, 1, 100);
  SKILL_ADD(CLASS_NECROMANCER, 1, 100);
  SKILL_ADD(CLASS_CABALIST, 1, 100);
  ...
  SKILL_ADD(CLASS_RANGER, 11, 80);
  SKILL_ADD(CLASS_BARD, 1, 50);
  SKILL_ADD(CLASS_REAVER, 11, 80);
  SKILL_ADD(CLASS_ILLUSIONIST, 1, 100);

  SKILL_CREATE("shamanic spell knowledge", SKILL_SPELL_KNOWLEDGE_SHAMAN, TAR_MENTAL);
  SKILL_ADD(CLASS_SHAMAN, 1, 100);
  ...
  SKILL_ADD(CLASS_PSIONICIST, 21, 80);
  ...
  SPELL_CREATE_MSG("elemental form", SPELL_ELEMENTAL_FORM, PULSE_SPELLCAST * 4, TAR_SELF_ONLY | TAR_NOCOMBAT,
 ...
 //SPEC_SPELL_ADD(CLASS_DRUID, 11, SPEC_STORM);
  ...
  SPELL_CREATE("binding wind", SPELL_BINDING_WIND, PULSE_SPELLCAST * 1, TAR_IGNORE | TAR_NOCOMBAT | TAR_ELEMENTAL, 
               spell_binding_wind);
  SPEC_SPELL_ADD(CLASS_ETHERMANCER, 9, SPEC_WINDTALKER);
  SPEC_SPELL_ADD(CLASS_DRUID, 10, SPEC_STORM);
  SPEC_SPELL_ADD(CLASS_SHAMAN, 11, SPEC_ELEMENTALIST);

  SPELL_CREATE_MSG("vampiric touch", SPELL_VAMPIRIC_TOUCH, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
                   spell_vampiric_touch, "Your hands cease to glow &+Rred.&n");
  SPELL_ADD(CLASS_NECROMANCER, 2);
  SPELL_ADD(CLASS_THEURGIST, 2);

  SPELL_CREATE("protect undead", SPELL_PROT_UNDEAD, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM, spell_prot_undead);
  SPELL_ADD(CLASS_NECROMANCER, 7);

  SPELL_CREATE("protect soul", SPELL_PROTECT_SOUL, PULSE_SPELLCAST * 2,  TAR_CHAR_ROOM, spell_prot_undead);
  SPELL_ADD(CLASS_THEURGIST, 7);
  ...
  SPELL_CREATE("command horde", SPELL_COMMAND_HORDE, 0, TAR_IGNORE, spell_command_horde);
  // SPELL_ADD(CLASS_NECROMANCER, 9);

  SPELL_CREATE("heal undead", SPELL_HEAL_UNDEAD, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM, spell_heal_undead);
  SPELL_ADD(CLASS_NECROMANCER, 5);
  SPELL_ADD(CLASS_WARLOCK, 5);

  SPELL_CREATE("mend soul", SPELL_MEND_SOUL, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM, spell_mend_soul);
  SPELL_ADD(CLASS_THEURGIST, 5);

  SPELL_CREATE("greater heal undead", SPELL_GREATER_HEAL_UNDEAD, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
  ...
  SPELL_CREATE_MSG("mass barkskin", SPELL_MASS_BARKSKIN, PULSE_SPELLCAST * 3, TAR_IGNORE,
  ...
  SPELL_CREATE("moonstone", SPELL_MOONSTONE, PULSE_SPELLCAST * 6, TAR_IGNORE | TAR_NOCOMBAT, spell_moonstone);
  SPELL_ADD(CLASS_DRUID, 12);

  SPELL_CREATE("create dracolich", SPELL_CREATE_DRACOLICH, PULSE_SPELLCAST * 9, TAR_OBJ_ROOM | TAR_NOCOMBAT,
               spell_create_dracolich);
  SPELL_ADD(CLASS_NECROMANCER, 11);

  SPELL_CREATE("call titan", SPELL_CALL_TITAN, PULSE_SPELLCAST * 9, TAR_OBJ_ROOM | TAR_NOCOMBAT,
               spell_call_titan);
  SPELL_ADD(CLASS_THEURGIST, 11);

  SPELL_CREATE("napalm", SPELL_NAPALM, PULSE_SPELLCAST * 4 / 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_napalm);

  SPELL_CREATE("strong acid", SPELL_STRONG_ACID, PULSE_SPELLCAST * 3/2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
               spell_strong_acid);

  SPELL_CREATE("glass bomb", SPELL_GLASS_BOMB, PULSE_SPELLCAST * 3/2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_glass_bomb);

  SPELL_CREATE("grease", SPELL_GREASE, PULSE_SPELLCAST * 3/2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_grease);

  SPELL_CREATE("nitrogen", SPELL_NITROGEN, PULSE_SPELLCAST * 3/2, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_nitrogen);

  SKILL_CREATE("garrote", SKILL_GARROTE, TAR_PHYS);
  SKILL_ADD(CLASS_ROGUE, 40, 50);
  SPEC_SKILL_ADD(CLASS_ROGUE, 20, 100, SPEC_ASSASSIN);
  SPEC_SKILL_ADD(CLASS_ROGUE, 31, 80, SPEC_THIEF);
  ...
  SKILL_ADD(CLASS_BARD, 1, 50);
  SPEC_SKILL_ADD(CLASS_ROGUE, 1, 100, SPEC_THIEF);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 41, 50, SPEC_SWASHBUCKLER);
  ...
  SPEC_SKILL_ADD(CLASS_WARRIOR, 41, 100, SPEC_SWASHBUCKLER);
  ...
  SKILL_CREATE_WITH_MESSAGES("throat crush", SKILL_THROAT_CRUSH, TAR_PHYS,
                             "Your throat ceases to hurt; you feel your voice returning...&n", NULL);
...
// Creating this bogus skill so only the person being throat crushed receives the wear off message.
  ...
  SKILL_ADD(CLASS_ALCHEMIST, 11, 85);
  ...
  SKILL_ADD(CLASS_RANGER, 46, 85);
  SKILL_ADD(CLASS_REAVER, 46, 80);
  SKILL_ADD(CLASS_MERCENARY, 51, 80);
  SKILL_ADD(CLASS_BERSERKER, 46, 70);
  SKILL_ADD(CLASS_PALADIN, 56, 85);
  SKILL_ADD(CLASS_ANTIPALADIN, 56, 85);
  ...
  SKILL_ADD(CLASS_ASSASSIN, 11, 80);
  SKILL_ADD(CLASS_ROGUE, 11, 85);
  SKILL_ADD(CLASS_MERCENARY, 1, 100);
  SKILL_ADD(CLASS_BERSERKER, 31, 80);
  SKILL_ADD(CLASS_REAVER, 1, 90);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 31, 100, SPEC_SWASHBUCKLER);
  SPEC_SKILL_ADD(CLASS_ROGUE, 11, 90, SPEC_ASSASSIN);
  SPEC_SKILL_ADD(CLASS_ROGUE, 46, 65, SPEC_THIEF);

  SKILL_CREATE("hitall", SKILL_HITALL, TAR_PHYS);
  SKILL_ADD(CLASS_WARRIOR, 1, 100);
  SKILL_ADD(CLASS_BERSERKER, 21, 85);
  SKILL_ADD(CLASS_DREADLORD, 31, 60);
  SKILL_ADD(CLASS_AVENGER, 31, 60);

  SKILL_CREATE("tackle", SKILL_TACKLE, TAR_PHYS);
  SKILL_ADD(CLASS_MERCENARY, 20, 100);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 41, 70, SPEC_SWASHBUCKLER);
  ...
  SKILL_CREATE_WITH_MESSAGES("infuriate", SKILL_INFURIATE, TAR_PHYS,
                             "&+RAs your R&+ra&+RG&+re&+R fades away, you return to your normal size.&n", NULL);
  SKILL_ADD(CLASS_BERSERKER, 41, 100);

  SKILL_CREATE("maul", SKILL_MAUL, TAR_PHYS);
  SKILL_ADD(CLASS_BERSERKER, 31, 90);
  ...
  SKILL_ADD(CLASS_WARRIOR, 1, 100);
  SKILL_ADD(CLASS_MERCENARY, 16, 60);
  SKILL_ADD(CLASS_CLERIC, 31, 60);
  SPEC_SKILL_ADD(CLASS_CLERIC, 21, 90, SPEC_ZEALOT);
  ...
  SKILL_ADD(CLASS_ALCHEMIST, 1, 60);
  SKILL_ADD(CLASS_BERSERKER, 1, 60);
  SKILL_ADD(CLASS_REAVER, 18, 60);
  SKILL_ADD(CLASS_DREADLORD, 1, 50);
  SKILL_ADD(CLASS_AVENGER, 1, 50);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 1, 100, SPEC_SWASHBUCKLER);
  SPEC_SKILL_ADD(CLASS_ROGUE, 1, 60, SPEC_ASSASSIN);
  ...
  SKILL_ADD(CLASS_ANTIPALADIN, 1, 75);
  SKILL_ADD(CLASS_RANGER, 21, 60);
  SKILL_ADD(CLASS_MERCENARY, 16, 70);
  SKILL_ADD(CLASS_WARRIOR, 1, 100);
  SKILL_ADD(CLASS_BERSERKER, 36, 50);
  SKILL_ADD(CLASS_REAVER, 21, 50);
  SKILL_ADD(CLASS_DREADLORD, 31, 50);
  SKILL_ADD(CLASS_AVENGER, 31, 50);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 36, 70, SPEC_BATTLE_FORGER);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 1, 100, SPEC_SWASHBUCKLER);

  SKILL_CREATE("surprise", SKILL_SURPRISE, 0);
  SKILL_ADD(CLASS_RANGER, 11, 100);

  SKILL_CREATE("headbutt", SKILL_HEADBUTT, TAR_PHYS);
  SKILL_ADD(CLASS_MERCENARY, 1, 100);

  SKILL_CREATE("double headbutt", SKILL_DOUBLE_HEADBUTT, TAR_PHYS);
  SKILL_ADD(CLASS_MERCENARY, 56, 75);
  ...
  SKILL_ADD(CLASS_PALADIN, 1, 75);
  SKILL_ADD(CLASS_RANGER, 1, 70);
  SKILL_ADD(CLASS_SHAMAN, 16, 50);
  SKILL_ADD(CLASS_ETHERMANCER, 26, 50);
  SKILL_ADD(CLASS_SORCERER, 31, 50);
  SKILL_ADD(CLASS_WARLOCK, 16, 70);
  SKILL_ADD(CLASS_ROGUE, 11, 85);
  SKILL_ADD(CLASS_THIEF, 11, 95);
  SKILL_ADD(CLASS_WARRIOR, 1, 75);
  SKILL_ADD(CLASS_MONK, 16, 80);
  SKILL_ADD(CLASS_ALCHEMIST, 21, 75);
  SKILL_ADD(CLASS_BERSERKER, 26, 50);
  SKILL_ADD(CLASS_REAVER, 1, 70);
  SKILL_ADD(CLASS_ILLUSIONIST, 26, 50);
  SKILL_ADD(CLASS_DREADLORD, 26, 50);
  SKILL_ADD(CLASS_AVENGER, 26, 50);
  ...
  SKILL_ADD(CLASS_ALCHEMIST, 1, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 15, 70, SPEC_BATTLE_FORGER);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 15, 70, SPEC_BLACKSMITH);

  SKILL_CREATE("throwpotion", SKILL_THROW_POTIONS, TAR_PHYS);
  SKILL_ADD(CLASS_ALCHEMIST, 1, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 15, 75, SPEC_BATTLE_FORGER);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 15, 75, SPEC_BLACKSMITH);

  SKILL_CREATE("remix", SKILL_REMIX, TAR_PHYS);
  SKILL_ADD(CLASS_ALCHEMIST, 51, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 51, 90, SPEC_BATTLE_FORGER);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 51, 75, SPEC_BLACKSMITH);

  SKILL_CREATE("fix", SKILL_FIX, TAR_PHYS);
  SKILL_ADD(CLASS_ALCHEMIST, 10, 100);

  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 10, 95, SPEC_BLACKSMITH);
  SKILL_CREATE("craft", SKILL_CRAFT, TAR_PHYS);

  SKILL_ADD(CLASS_ALCHEMIST, 15, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 15, 90, SPEC_BATTLE_FORGER);

  SKILL_CREATE("encrust", SKILL_ENCRUST, TAR_PHYS);
  SKILL_ADD(CLASS_ALCHEMIST, 20, 100);

  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 20, 75, SPEC_BATTLE_FORGER);
  SKILL_CREATE("smelt", SKILL_SMELT, TAR_PHYS);

  SKILL_ADD(CLASS_ALCHEMIST, 25, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 25, 100, SPEC_BLACKSMITH);

  SKILL_CREATE("forge", SKILL_FORGE, TAR_MENTAL);
  SKILL_ADD(CLASS_ALCHEMIST, 30, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 30, 100, SPEC_BLACKSMITH);

  SKILL_CREATE("enchant", SKILL_ENCHANT, TAR_PHYS);
  SKILL_ADD(CLASS_ALCHEMIST, 40, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 40, 90, SPEC_BLACKSMITH);

  SKILL_CREATE("spellbind", SKILL_SPELLBIND, TAR_PHYS);
  SKILL_ADD(CLASS_ALCHEMIST, 50, 100);
  SPEC_SKILL_ADD(CLASS_ALCHEMIST, 50, 75, SPEC_BATTLE_FORGER);
  ...
  SPELL_CREATE("aid of the heavens", SPELL_AID_OF_THE_HEAVENS, PULSE_SPELLCAST * 2, TAR_AREA | TAR_AGGRO, 
               spell_aid_of_the_heavens);
  SPELL_ADD(CLASS_THEURGIST, 11);

  SPELL_CREATE("ghastly touch", SPELL_GHASTLY_TOUCH, PULSE_SPELLCAST * 2 / 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, 
               spell_ghastly_touch);
 ...
 /* Epic Point Skills */
  ...
  TAG_CREATE_WITH_MESSAGES("vampire bite", TAG_VAMPIRE_BITE,
                             "The nauseous feeling from the bitten neck fades...", "");
  ...
  TAG_CREATE("umberlees fury target", TAG_UMBERLEES_FURY_TARGET);
